function [MF, nums, shape, FSL, FSR] = HMAWithoutExtensiveRemoval(L, R, inputShape)
%
...
elseif  (shape == 3)
    ...
    if (sum(L - switchPoint) == 0)
        UMF = [switchPoint, switchPoint, 10, 10];
        LMF = [switchPoint, switchPoint, 10, 10, 1];
    ...
    UMF = [al, switchPoint, 10, 10];
    LMF = [ar, switchPoint, 10, 10, 1];
else
    overlapLeft = max(L);
    overlapRight = min(R);
